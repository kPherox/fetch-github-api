...
module.exports = class FetchGitHubApi {
    ...
    get basicAuth() {
        if (this.username)
            return base64.encode(this.username + ":" + this.password ? this.password : "");

        return null;
    }

    set basicAuth(arg) {
        let basicAuth = arg.split(':');

        this.username = basicAuth[0];
        this.password = basicAuth[1];
    }
    ...
    fetchApi(pageNumber = 0) {
        ...
        if (this.totpToken)
            headers.append('X-GitHub-OTP', this.totpToken);
